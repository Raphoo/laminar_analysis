...
for k = [3 4 6]
    ...
    for kk = 1 : length(good_sess)
        ...
        load('LFP_1k.mat')
        load('States.mat')
        cfg=[]; cfg.channel = data.label(good_ctx_th_ch); data1=ft_preprocessing(cfg,data);
        times_all = findspindlesv5(data,1000,9,17,time_STATE2gs(states(1).t),2.4);
        save('spinddelt_cpling_spindlesdetected.mat','times_all')
        ...
        dphsa{dphsa_ind} = dphs; dphsa_ind = dphsa_ind+1;
        save('spinddelt_cpling_glm_sess.mat','dphs','pv','z')
        ...
        [wxa1,wva1] = GLMSpindlePark(data,unitdata,st,good_ctx_th_ch,cds(1:round(length(cds)./4)));
        [wxa2,wva2] = GLMSpindlePark(data,unitdata,st,good_ctx_th_ch,cds((round(length(cds).*.75)+1):end));
        else
