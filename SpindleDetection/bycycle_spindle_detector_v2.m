function [st_all,trgh_times_all] = bycycle_spindle_detector_v2(bycycle_params,states,fd,nch)
...
mkdir spindle_detect

% run spindle detector
system(['python C:\Users\Loomis\Documents\Scripts\spindle_detect_bycycle.py ' ['"' fd '\LFP_DS_1k.mat"'] ' ' num2str(bycycle_params(1)) ...
    ' ' num2str(bycycle_params(2)) ' ' num2str(bycycle_params(3)) ' ' num2str(bycycle_params(4))])
% Import Table
st_all = {}; trgh_times_all = {};
gs=time_STATE2gs(states(1).t);
for k = 0:(nch-1)
spind_table=readtable([fd '\spindle_detect\spindle_peaks' num2str(k) '.csv']);
...
%IMPORTANT
st_all{k+1} = st; trgh_times_all{k+1}=trgh_times;
disp(k)
end
save([fd '\bycycle_spindledetect_out.mat'],'st_all','trgh_times_all')
end