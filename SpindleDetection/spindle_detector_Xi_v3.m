function [st,spr] = spindle_detector_Xi_v3(lfp,gs)
...
cfg=[]; cfg.bpfilter='yes'; cfg.bpfreq=[7.5 17]; spind = ft_preprocessing(cfg,lfp);
...
st = cell(1,size(lfp.trial{1},1)); spr = cell(1,size(lfp.trial{1},1));
parfor k = 1 : size(lfp.trial{1},1)
    br=[];
