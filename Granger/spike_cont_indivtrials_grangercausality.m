function [F,tr] = spike_cont_indivtrials_grangercausality(unitdata,tlen,fs,toilim)
...
F = zeros(size(X,1),size(X,1),size(X,3));
...
ptic('\n*** tsdata_to_var... ');
[A,SIG] = tsdata_to_var(X(:,:,k),morder,regmode);
ptoc;
