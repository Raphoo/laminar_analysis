function [PACma, PACzma, ModIndma, ModIndZma, ModIndNullma] = LFP_MIv3(lfp,phsb,ampb,nPerms,gs)
...
parfor ch_ind=1:nch
    ...
    inds=inds(1:round(length(inds)*.2));
     ...
     p=pa(ch_ind,inds);
     hatmp = ha(:,inds,:);
     [PACm, PACzm, ModIndm, ModIndZm, ModIndNullm] = miv3_subfun(p,hatmp,ampb,nPerms,nch);
...
function [PACm, PACzm, ModIndm, ModIndZm, ModIndNullm] = miv3_subfun(p,ha,ampb,nPerms,nch)
    PACm=ones(size(ha,1),37,size(ampb,1)).*eps;
    p = p * 360.0 / (2.0*pi);                 %Use phase in degrees.
      ...
      PACzm=ones(nch,37,nPerms,size(ampb,1)).*eps;
      for perm=1:nPerms
