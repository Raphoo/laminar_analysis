...
function all_edge = unit_burst_detector(unitdata,fqs,cent_crit, cent_time, edge_crit, edge_time, time_inds, chs)

% derive MUA

u = unitdata_to_array(unitdata);

if isempty(time_inds)
    time_inds = 1:length(u);
end
if isempty(chs)
    chs = 1:size(u,2);
end
